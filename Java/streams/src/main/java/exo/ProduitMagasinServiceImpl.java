...
public class ProduitMagasinServiceImpl implements ProduitMagasinService{
    ...
    public boolean deleteproduct(int idmagasin, int idproduit) {
        boolean rslt = false;
        ProduitMagasin magTodelete;

        System.out.println(listeproduitmagasin);
        if (listeproduitmagasin.stream().filter(p -> p.getIdMagasin() == idmagasin && p.getIdProduit() == idproduit).count() != 0){
            magTodelete = listeproduitmagasin.stream().filter(p -> p.getIdMagasin() == idmagasin && p.getIdProduit() == idproduit).findFirst().get();
            magTodelete.getIdMagasin();
            magTodelete.getIdProduit();
            rslt = listeproduitmagasin.remove(magTodelete);
            System.out.println(listeproduitmagasin);
        }
        return rslt;
    }
